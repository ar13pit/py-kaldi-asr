...
def find_dependencies():
    ...
    if status != 0:
        ...
        for libdir in [
            ...
            if os.path.isfile('%s/libatlas.so' % libdir):
                found = True
                break
        if not found:
            raise Exception('Failed to find libatlas.so.3 on your system.')

        kw.setdefault('libraries', []).append('%s/atlas.so' % libdir)
    ...
    status, output = getstatusoutput(
        ["pkg-config", "--libs", "--cflags", "kaldi"])

    if status != 0:
        raise Exception("*** failed to find pkgconfig for kaldi")
...
ext_modules += [
    Extension("kaldiasr.nnet3",
              ...
              runtime_library_dirs=["/home/arpit/src/kaldi_speech/src/lib"],
              **find_dependencies()),
